...
import {
    ...
    scrapeEuropaCriativa as scrapeEuropaCriativaCore,
    extractWithGemini,
...
async function scrapeEuropaCriativa(input: Input): Promise<Aviso[]> {
    try {
        const coreAvisos = await scrapeEuropaCriativaCore({
            maxItems: input.maxItemsPerPortal,
            onlyOpen: input.onlyOpen,
            includeDocuments: input.includeDocuments,
            maxDocumentsPerAviso: 25,
        });
        ...
        if (coreAvisos.length > 0) {
            return coreAvisos.map(a => ({
                ...
                dataFecho: a.dataFecho,
                montante: a.dotacao ? String(a.dotacao) : undefined,
                regiao: Array.isArray(a.regiao) ? a.regiao.join(', ') : (a.regiao || 'Europa'),
                url: a.url,
                pdfUrl: a.documentos?.find(d => (d.tipo || '').toLowerCase().includes('pdf'))?.url,
                status: a.status,
                scrapedAt: a.scrapedAt,
                documentos: a.documentos?.map(d => ({
                    id: d.id || '',
        ...
        console.log(`    ⚠️ Europa Criativa core falhou: ${(err as Error).message}`);
    }
