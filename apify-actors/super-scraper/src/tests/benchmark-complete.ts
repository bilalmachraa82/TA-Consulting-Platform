...
async function runCompleteBenchmark(): Promise<void> {
    ...
    // Build context - TRUNCATE TO AVOID 400 ERROR
    const limit = 30; // Limit context to 30 items for benchmark stability
    const contextAvisos = allAvisos.slice(0, limit);
    let context = `=== ${contextAvisos.length} AVISOS ABERTOS (Limitado de ${allAvisos.length}) ===\n\n`;
    for (const a of contextAvisos) {
    ...
    for (const modelId of MODELS_TO_TEST) {
        ...
        for (const q of QUESTIONS) {
            try {
                ...
                console.log(`  ❌ ${q.id}: Full Error: ${JSON.stringify(e, Object.getOwnPropertyNames(e))}`);
            }
        }
    }
