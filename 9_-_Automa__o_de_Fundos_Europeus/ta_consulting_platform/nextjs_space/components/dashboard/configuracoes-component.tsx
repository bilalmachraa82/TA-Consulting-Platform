...
const SECOES_CONFIGURACAO: ConfiguracaoSecao[] = [
  ...
  },
  {
    ...
    descricao: 'Configuração de API keys para IA e LLMs'
  },
  {
    id: 'scraping',
...
export function ConfiguracoesComponent() {
  ...
  const [configuracoes, setConfiguracoes] = useState<any>({
    ...
    apiKeys: {
      providerAtivo: 'ABACUS_AI',
      abacusApiKey: '',
      anthropicApiKey: '',
      openaiApiKey: '',
      chatbotModel: 'claude-4-5-haiku',
      memoriaModel: 'claude-4-5-sonnet',
      usarDefaultKey: true
    },
    scraping: {
      portugal2030: true,
  ...
  const renderSecaoApiKeys = () => (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle>Configuração de API Keys LLM</CardTitle>
          <CardDescription>
            Configure suas próprias API keys para usar modelos de IA. 
            Durante o teste, usamos credenciais default (Abacus AI).
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-center justify-between mb-3">
              <div>
                <Label className="font-semibold">Usar API Key Default (Teste)</Label>
                <p className="text-sm text-gray-600 mt-1">
                  Enquanto ativado, a plataforma usa credenciais do desenvolvedor.
                  <br />
                  Desative para usar sua própria API key.
                </p>
              </div>
              <Switch
                checked={configuracoes.apiKeys.usarDefaultKey}
                onCheckedChange={(checked) => atualizarConfiguracao('apiKeys', 'usarDefaultKey', checked)}
              />
            </div>
            
            {configuracoes.apiKeys.usarDefaultKey && (
              <div className="text-sm text-blue-700 bg-blue-100 p-3 rounded">
                ℹ️ <strong>Modo Teste Ativo:</strong> Usando API key do desenvolvedor. 
                Todas as funcionalidades de IA estão disponíveis sem custo adicional durante o período de teste.
              </div>
            )}
          </div>
          ...
          {!configuracoes.apiKeys.usarDefaultKey && (
            <>
              <div>
                ...
                <Select 
                  value={configuracoes.apiKeys.providerAtivo}
                  onValueChange={(value) => atualizarConfiguracao('apiKeys', 'providerAtivo', value)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="ABACUS_AI">Abacus AI (Recomendado)</SelectItem>
                    <SelectItem value="ANTHROPIC">Anthropic (Claude direto)</SelectItem>
                    <SelectItem value="OPENAI">OpenAI (GPT)</SelectItem>
                  </SelectContent>
                </Select>
                <p className="text-sm text-gray-500 mt-1">
                  Abacus AI oferece acesso a múltiplos modelos com preços competitivos
                </p>
              </div>

              {/* API Keys */}
              <div className="space-y-4 pt-4 border-t">
                {configuracoes.apiKeys.providerAtivo === 'ABACUS_AI' && (
                  <div>
                    <Label htmlFor="abacusKey">Abacus AI API Key</Label>
                    <Input
                      id="abacusKey"
                      type="password"
                      value={configuracoes.apiKeys.abacusApiKey}
                      onChange={(e) => atualizarConfiguracao('apiKeys', 'abacusApiKey', e.target.value)}
                      placeholder="abacus_xxxxxxxxxxxxx"
                    />
                    <p className="text-sm text-gray-500 mt-1">
                      Obtenha em: <a href="https://abacus.ai" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">abacus.ai</a>
                    </p>
                  </div>
                )}

                {configuracoes.apiKeys.providerAtivo === 'ANTHROPIC' && (
                  <div>
                    <Label htmlFor="anthropicKey">Anthropic API Key</Label>
                    <Input
                      id="anthropicKey"
                      type="password"
                      value={configuracoes.apiKeys.anthropicApiKey}
                      onChange={(e) => atualizarConfiguracao('apiKeys', 'anthropicApiKey', e.target.value)}
                      placeholder="sk-ant-xxxxxxxxxxxxx"
                    />
                    <p className="text-sm text-gray-500 mt-1">
                      Obtenha em: <a href="https://console.anthropic.com" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">console.anthropic.com</a>
                    </p>
                  </div>
                )}

                {configuracoes.apiKeys.providerAtivo === 'OPENAI' && (
                  <div>
                    <Label htmlFor="openaiKey">OpenAI API Key</Label>
                    <Input
                      id="openaiKey"
                      type="password"
                      value={configuracoes.apiKeys.openaiApiKey}
                      onChange={(e) => atualizarConfiguracao('apiKeys', 'openaiApiKey', e.target.value)}
                      placeholder="sk-xxxxxxxxxxxxx"
                    />
                    <p className="text-sm text-gray-500 mt-1">
                      Obtenha em: <a href="https://platform.openai.com" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">platform.openai.com</a>
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t">
                <div>
                  <Label htmlFor="chatbotModel">Modelo Chatbot</Label>
                  <Select 
                    value={configuracoes.apiKeys.chatbotModel}
                    onValueChange={(value) => atualizarConfiguracao('apiKeys', 'chatbotModel', value)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="claude-4-5-haiku">Claude 4.5 Haiku (Rápido)</SelectItem>
                      <SelectItem value="claude-4-5-sonnet">Claude 4.5 Sonnet (Balanceado)</SelectItem>
                      <SelectItem value="gpt-4o-mini">GPT-4o Mini</SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-sm text-gray-500 mt-1">Para respostas rápidas</p>
                </div>

                <div>
                  <Label htmlFor="memoriaModel">Modelo Memórias Descritivas</Label>
                  <Select 
                    value={configuracoes.apiKeys.memoriaModel}
                    onValueChange={(value) => atualizarConfiguracao('apiKeys', 'memoriaModel', value)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="claude-4-5-sonnet">Claude 4.5 Sonnet (Recomendado)</SelectItem>
                      <SelectItem value="claude-4">Claude 4.0</SelectItem>
                      <SelectItem value="gpt-4o">GPT-4o</SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-sm text-gray-500 mt-1">Para documentos longos</p>
                </div>
              </div>
            </>
          )}
      ...
      <Card className="border-yellow-200 bg-yellow-50">
        <CardHeader>
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            Custos Estimados (modo produção)
          </CardTitle>
        </CardHeader>
        <CardContent className="text-sm text-gray-700">
          <ul className="space-y-2">
            <li>• <strong>Chatbot:</strong> ~€0.01-0.03 por conversa</li>
            <li>• <strong>Memória Descritiva:</strong> ~€0.20-0.30 por documento</li>
            <li>• <strong>Recomendações IA:</strong> ~€0.005-0.01 por análise</li>
          </ul>
          <p className="mt-3 text-xs text-gray-500">
            Valores aproximados usando Claude 4.5 via Abacus AI. 
            Custos reais dependem do tamanho dos documentos e complexidade.
          </p>
        </CardContent>
      </Card>
    </div>
              ...
              {SECOES_CONFIGURACAO.map((secao, index) => (
                <motion.button
                  ...
                  className={`w-full text-left p-3 flex items-center gap-3 hover:bg-gray-50 transition-colors ${
                    secaoAtiva === secao.id ? 'bg-blue-50 text-blue-700 border-r-2 border-blue-700' : ''
                  }`}
                >
                  {secao.icone}
                  <div>
