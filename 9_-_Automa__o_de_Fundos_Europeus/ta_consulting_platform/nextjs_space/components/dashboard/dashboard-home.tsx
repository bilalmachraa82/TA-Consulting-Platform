...
interface AlertasDocumentos {
  resumo: {
    total: number;
    expirados: number;
    expiramEm7Dias: number;
    expiramEm15Dias: number;
...
export function DashboardHome({ kpis, avisos, candidaturas, notificacoes, workflows }: DashboardHomeProps) {
  ...
  const [alertasDocumentos, setAlertasDocumentos] = useState<AlertasDocumentos | null>(null);
  const [loadingAlertas, setLoadingAlertas] = useState(true);

  useEffect(() => {
    fetchMetricas();
    fetchAlertasDocumentos();
  }, []);
  ...
  const fetchAlertasDocumentos = async () => {
    try {
      const response = await fetch('/api/documentos/alertas');
      const data = await response.json();
      if (data.success) {
        setAlertasDocumentos(data);
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {!loadingAlertas && alertasDocumentos && alertasDocumentos.resumo.total > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <Card className="border-l-4 border-l-red-500 bg-red-50">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle className="flex items-center gap-2 text-red-700">
                    ...
                    Alertas de Documentos ({alertasDocumentos.resumo.total})
                  </CardTitle>
                  <CardDescription>Documentos que requerem atenção urgente</CardDescription>
                </div>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {/* Expirados */}
                {alertasDocumentos.resumo.expirados > 0 && (
                  <div className="bg-white p-4 rounded-lg border-2 border-red-300">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-red-700">🔴 Expirados</h4>
                      <Badge variant="destructive">{alertasDocumentos.resumo.expirados}</Badge>
                    </div>
                    <div className="space-y-2 max-h-32 overflow-y-auto">
                      {alertasDocumentos.alertas.expirados.slice(0, 3).map((doc: any) => (
                        <div key={doc.id} className="text-sm">
                          <p className="font-medium truncate">{doc.empresa.nome}</p>
                          <p className="text-xs text-gray-600">{doc.nome}</p>
                        </div>
                      ))}
                      {alertasDocumentos.alertas.expirados.length > 3 && (
                        <p className="text-xs text-gray-500">+{alertasDocumentos.alertas.expirados.length - 3} mais...</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                {/* Expiram em 7 dias */}
                {alertasDocumentos.resumo.expiramEm7Dias > 0 && (
                  <div className="bg-white p-4 rounded-lg border-2 border-orange-300">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-orange-700">🟠 ≤ 7 dias</h4>
                      <Badge className="bg-orange-500">{alertasDocumentos.resumo.expiramEm7Dias}</Badge>
                    </div>
                    <div className="space-y-2 max-h-32 overflow-y-auto">
                      {alertasDocumentos.alertas.expiramEm7Dias.slice(0, 3).map((doc: any) => (
                        <div key={doc.id} className="text-sm">
                          <p className="font-medium truncate">{doc.empresa.nome}</p>
                          <p className="text-xs text-gray-600">{doc.nome}</p>
                        </div>
                      ))}
                      {alertasDocumentos.alertas.expiramEm7Dias.length > 3 && (
                        <p className="text-xs text-gray-500">+{alertasDocumentos.alertas.expiramEm7Dias.length - 3} mais...</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                {/* Expiram em 15 dias */}
                {alertasDocumentos.resumo.expiramEm15Dias > 0 && (
                  <div className="bg-white p-4 rounded-lg border-2 border-yellow-300">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-yellow-700">🟡 ≤ 15 dias</h4>
                      <Badge className="bg-yellow-500">{alertasDocumentos.resumo.expiramEm15Dias}</Badge>
                    </div>
                    <div className="space-y-2 max-h-32 overflow-y-auto">
                      {alertasDocumentos.alertas.expiramEm15Dias.slice(0, 3).map((doc: any) => (
                        <div key={doc.id} className="text-sm">
                          <p className="font-medium truncate">{doc.empresa.nome}</p>
                          <p className="text-xs text-gray-600">{doc.nome}</p>
                        </div>
                      ))}
                      {alertasDocumentos.alertas.expiramEm15Dias.length > 3 && (
                        <p className="text-xs text-gray-500">+{alertasDocumentos.alertas.expiramEm15Dias.length - 3} mais...</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                {/* Expiram em 30 dias */}
                {alertasDocumentos.resumo.expiramEm30Dias > 0 && (
                  <div className="bg-white p-4 rounded-lg border-2 border-blue-300">
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-blue-700">🔵 ≤ 30 dias</h4>
                      <Badge className="bg-blue-500">{alertasDocumentos.resumo.expiramEm30Dias}</Badge>
                    </div>
                    <div className="space-y-2 max-h-32 overflow-y-auto">
                      {alertasDocumentos.alertas.expiramEm30Dias.slice(0, 3).map((doc: any) => (
                        <div key={doc.id} className="text-sm">
                          <p className="font-medium truncate">{doc.empresa.nome}</p>
                          <p className="text-xs text-gray-600">{doc.nome}</p>
                        </div>
                      ))}
                      {alertasDocumentos.alertas.expiramEm30Dias.length > 3 && (
                        <p className="text-xs text-gray-500">+{alertasDocumentos.alertas.expiramEm30Dias.length - 3} mais...</p>
                      )}
                    </div>
                  </div>
                )}
