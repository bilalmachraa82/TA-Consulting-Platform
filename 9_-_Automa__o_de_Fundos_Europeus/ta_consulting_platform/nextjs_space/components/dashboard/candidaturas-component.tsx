                      ...
                      <CardContent className="space-y-3">
                        ...
                        <div className="flex gap-2 pt-2 border-t">
                          ...
                          <Button
                            ...
                            onClick={() => window.location.href = `/dashboard/memorias-descritivas?candidatura=${candidatura.id}`}
                            title="Gerar Memória Descritiva com IA"
                          >
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-3 pt-4 border-t">
                ...
                <Button 
                  ...
                  onClick={async () => {
                    try {
                      const response = await fetch(`/api/candidaturas/${candidaturaSelecionada.id}/clonar`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({}),
                      });
                      ...
                      if (data.success) {
                        toast.success('Candidatura clonada com sucesso!');
                        fetchCandidaturas();
                        setModalDetalhes(false);
                      } else {
                        toast.error(data.error || 'Erro ao clonar candidatura');
                      }
                    } catch (error) {
                      toast.error('Erro ao clonar candidatura');
                    }
                  }}
                >
                  Clonar Candidatura
                ...
                <Button 
                  variant="outline"
                  onClick={async () => {
                    const nomeTemplate = prompt('Nome do template:');
                    if (!nomeTemplate) return;
                    
                    try {
                      const response = await fetch(`/api/candidaturas/${candidaturaSelecionada.id}/salvar-template`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ 
                          nomeTemplate,
                          descricao: `Template criado a partir de ${candidaturaSelecionada.aviso.nome}`,
                          isPublico: false,
                        }),
                      });
                      const data = await response.json();
