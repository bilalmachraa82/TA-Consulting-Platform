...
export interface ScrapedAvisoRecord {
  ...
  url?: string;
  status?: string;
  tipo_beneficiario?: string | string[];
...
function normalizePortal(fonte?: string): 'PORTUGAL2030' | 'PEPAC' | 'PRR' {
  const normalized = (fonte || '').toUpperCase();

  if (normalized.includes('PORTUGAL')) return 'PORTUGAL2030';
  if (normalized.includes('PEPAC') || normalized.includes('PAPAC')) return 'PEPAC';
...
function normalizeEligibleCompanySizes(tipoBeneficiario?: string | string[]): string[] {
  const normalized = joinText(tipoBeneficiario).toLowerCase();
...
function derivePrograma(record: ScrapedAvisoRecord): string {
  ...
  return record.fonte?.trim() || joinText(record.setor).trim() || 'Programa Geral';
}
...
export function normalizeScrapedAviso(
  ...
  return {
    ...
    linha: joinText(record.tipo_beneficiario).trim() || null,
    ...
    descricao: record.descricao?.trim() || null,
    link: record.url?.trim() || null,
    taxa: record.taxa_apoio ? `${record.taxa_apoio}%` : null,
    regiao: joinText(record.regiao).trim() || null,
    setoresElegiveis: toList(record.setor),
