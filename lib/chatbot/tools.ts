...
export function buildAvisosWhere(params: SearchAvisosParams, now: Date = new Date()): Prisma.AvisoWhereInput {
    ...
    if (params.apenasAbertos) {
        where.dataFimSubmissao = { gte: now };
    }
...
export async function executeSearchAvisos(rawParams: unknown): Promise<{ resultados: unknown[]; citations: AvisoCitation[] }> {
    const params = searchAvisosParams.parse(rawParams);
    const avisos = await prisma.aviso.findMany({
        where: buildAvisosWhere(params),
        select: AVISO_SELECT,
        orderBy: { dataFimSubmissao: 'asc' },
        take: Math.min(params.limite, MAX_RESULTADOS),
    });

    const citations: AvisoCitation[] = avisos.map((a) => ({
        codigo: a.codigo,
        nome: a.nome,
        portal: String(a.portal),
        link: a.link,
        prazo: a.dataFimSubmissao.toISOString().slice(0, 10),
    }));
