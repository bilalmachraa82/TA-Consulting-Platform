...
export async function uploadLocalFileToGeminiFilesApi(
    ...
    let resolvedMimeType = mimeType || guessMimeType(filePath);
    let finalDisplayName = displayName;

    // OPTIMIZATION: Convert PDF to Text (Deep Text Extraction)
    ...
    if (resolvedMimeType === 'application/pdf') {
        try {
            ...
            // Basic cleanup: remove excessive newlines and whitespace
            ...
            if (cleanedText.length > 100) {
                console.log(`    ✨ PDF Optimization: Extracted ${cleanedText.length} chars (was ${buffer.length} bytes)`);
                buffer = Buffer.from(cleanedText, 'utf-8');
                resolvedMimeType = 'text/plain';
                // We keep the displayName but the content is now text. 
                // Gemini handles text/plain very well.
            } else {
                console.log(`    ⚠️ PDF Text Extraction too small (${cleanedText.length} chars), keeping original.`);
            }
        } catch (e: any) {
            console.warn(`    ⚠️ PDF Optimization failed for ${displayName}, uploading original. Error: ${e.message}`);
        }
    }
...
function summarizeCitations(groundingMetadata: any): FileSearchCitationSummary {
    ...
    return {
        ...
        citedSources: Array.from(unique.values()),
    };
}
