...
export async function POST(req: Request) {
    try {
        ...
        const { portal, forceQuality = false } = body;

        const config = ENHANCED_PORTAL_CONFIGS[portal];
        if (!config) {
        ...
        if (!hasValidData) {
            ...
            // Tentar browser automation para portais críticos
            if (['portugal2030', 'prr', 'pepac', 'ipdj'].includes(portal)) {
                try {
