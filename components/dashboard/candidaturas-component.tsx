...
import { MilestonesTab } from '@/components/dashboard/post-award/milestones-tab'
import { PagamentosTab } from '@/components/dashboard/post-award/pagamentos-tab'
import {
  FileText,
  Building2,
  AlertTriangle,
...
import { AIWriterPanel } from '@/components/candidatura/ai-writer-panel'
    ...
    <div className="space-y-6">
      ...
      <Dialog open={modalDetalhes} onOpenChange={setModalDetalhes}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-slate-900/95 backdrop-blur-xl border-slate-700">
          ...
          {candidaturaSelecionada && (
            <div className="space-y-6">
              {/* V2.0: Tabs for APROVADA candidaturas */}
              {candidaturaSelecionada.estado === 'APROVADA' ? (
                <Tabs defaultValue="resumo" className="w-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      {candidaturaSelecionada.montanteAprovado && (
                        <Card className="bg-green-50 border-green-200">
                          <CardHeader className="pb-3">
                            <CardTitle className="text-sm text-green-700">Montante Aprovado</CardTitle>
                          </CardHeader>
                          <CardContent>
                  ...
                  <TabsContent value="milestones" className="mt-4">
                    <MilestonesTab candidaturaId={candidaturaSelecionada.id} />
                  </TabsContent>

                  <TabsContent value="pagamentos" className="mt-4">
                    <PagamentosTab candidaturaId={candidaturaSelecionada.id} />
                  </TabsContent>
                </Tabs>
              ) : (
                /* Original content for non-APROVADA candidaturas */
                <>
                  {/* Status e Montante */}
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card>
              ...
              {/* PDF Export Floating Action */}
              <div className="absolute top-6 right-16">
                <PDFExportDialog
                  entityId={candidaturaSelecionada.id}
                  entityType="candidatura"
                  entityName={`${candidaturaSelecionada.empresa.nome} - ${candidaturaSelecionada.aviso.nome}`}
                />
              </div>
            </div>
          )}
      ...
      {candidaturaSelecionada && (
        <AIWriterPanel
          ...
          empresaId={candidaturaSelecionada.empresa.nipc} // Fallback to NIPC if ID not available or matched
          avisoId={candidaturaSelecionada.aviso.codigo} // Fallback to code
          onApplyContent={(content) => {
            // Function to handle content application
            // For MVP: copy to clipboard and toast
